...
consumable --}}
                <div class="card-body">
                    <form action="{{ route('submit_user') }}" method="POST">
                        ...
                        <select class="form-select form-select-lg mb-3 text-capitalize" id="role" name="role" aria-label="role">
                            {{-- <option selected>Status</option> --}}
                            <option {{ $data->role == 'admin' ?  "selected":'' }} class="text-capitalize" value="admin">admin</option>
                            <option {{ $data->role == 'staff gudang' ?  "selected":'' }} class="text-capitalize" value="staff gudang">Staff Gudang</option>
                            <option {{ $data->role == 'staff' ?  "selected":'' }} class="text-capitalize" value="staff">Staff</option>
                            {{-- <option value="Lost">Lost</option> --}}
                        </select>
                        <label for="status" class="form-label text-capitalize">status</label>
                        <select class="form-select form-select-lg mb-3 text-capitalize" id="status" name="status" aria-label="Status">
                            {{-- <option selected>Status</option> --}}
                            <option {{ $data->status == 'aktif' ?  "selected":'' }} class="text-capitalize" value="aktif">Aktif</option>
                            <option {{ $data->status == 'inactive' ?  "selected":'' }} class="text-capitalize" value="inactive">inactive</option>
